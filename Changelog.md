...
* **增强：** 适配 komga 的新 next-ui以及 kmrs 的 kmweb
...
* **修复：** Safari 获取 Cookie 失败

### 版本 2.9.17

* **修复：** One-Shot 类型封面更新不正确
...
* **增强：** 优化系列元素查找逻辑并修复 ID 属性命名不一致问题

### 版本 2.9.14

* **增强：** 添加设置允许批量匹配时强制刷新含有 Btv 链接的条目

### 版本 2.9.13

* **修复：** Bangumi 作者信息提取存在问题

### 版本 2.9.12

* **杂项：** 进一步优化 Bangumi 作者和出版社相关信息的提取

### 版本 2.9.11

* **增强：** 添加设置允许控制是否获取单行本数据
* **杂项：** 优化 Bangumi 出版社信息提取
...
* **增强：** 添加 Mangadex 源用于封面更新
* **杂项：** 避免重复上传相同的封面图像

### 版本 2.9.7

* **增强：** 优化 Bangumi 单行本元数据提取逻辑
...
* **增强：** 匹配成功的系列从手动匹配收藏夹中自动移除
* **修复：** 收藏和阅读列表中的刮削按钮顺序不正确
* **杂项：** 不再将类型写入 tag
* **杂项：** 继续优化封面图像上传逻辑
...
* **增强：** 处理从 Bangumi 标签元数据中提取的多种连载状态

### 版本 2.8.5

* **增强：** 进一步改进 Komga api 的使用
* **增强：** 批量匹配中成功的系列将尝试从手动匹配收藏夹中自动移除
* **增强：** 非收藏夹页面执行批量匹配时跳过已在手动匹配收藏夹中的系列
* **增强：** 优化批量匹配的底部进度条的显示和刷新逻辑
...
* **增强：** 从 Bangumi 元数据中提取分类信息
* **增强：** 从 Bangumi 标签元数据中提取可能的连载状态
* **增强：** 从 Bangumi 元数据中提取更多标签信息

### 版本 2.7.8

* **增强：** 尝试从系列文件夹名称中提取更多有用信息

### 版本 2.7.7

* **增强：** 为 Moe 来源添加缺失的出版社信息

### 版本 2.7.6

* **增强：** 尝试从系列文件夹名称中提取出版社/汉化信息

### 版本 2.7.5

* **增强：** 添加对 one-shot 类型的元数据刮削支持
* **增强：** 批量匹配操作现默认按最后入库顺序执行
* **增强：** 优化按钮注入实现
* **修复：** 元数据刮削按钮不在 one-shot 类型上显示

### 版本 2.7.0

* **增强：** 添加设置项允许选择匹配目标：漫画或小说
* **增强：** 添加更多 Komga API 操作
* **增强：** 为批量匹配添加底部进度条显示
* **增强：** 支持收藏夹内的批量匹配操作
* **增强：** 在库推荐页面执行批量匹配将自动匹配最近添加和更新的系列（30 个条目）
* **增强：** 卷元数据获取逻辑优化，避免不必要的请求
* **调整：** 整理代码功能分类
* **修复：** 脚本提示信息截断较长内容的显示
...
* **增强：** 改用 Komga 新 API 接口获取系列和书籍列表
* **增强：** Bangumi 单行本卷号识别
* **增强：** Bangumi 漫画作者/原作信息缺失时使用脚本·分镜或作画信息补充
* **修复：** 单行本更新判断不正确
